...
_svc = FyersService(app_id=APP_ID, secret=SECRET, token=TOKEN, redirect_url=REDIRECT_URL)
...
def backtest():
    ...
    for day in candles:
        S = day["c"]; iv = 0.13 + random.random() * 0.06
        atm = round(S / 100) * 100; T = 7 / 365; r = 0.065
        ...
        if strategy in ["straddle", "strangle", "ironCondor"]:
            pnl = max(min(
                prem * lot_size * (0.6 if abs(move) < 0.012 else -0.4) * (0.5 + random.random()),
            ...
            pnl = max(min((ev - abs(prem)) * lot_size, abs(prem) * tgt_pct / 100 * lot_size),
    ...
    eq = 0; equity = []
    for t in trades:
        eq += t["pnl"]; equity.append({"date": t["date"], "equity": round(eq, 2)})
    return jsonify({
        "success": True,
        "summary": {
            "total": tot, "wins": len(wins), "losses": len(losses),
